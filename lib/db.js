...
export async function getDb() {
  ...
  // During Next.js static build phase, return a dummy DB object to ensure zero DB/binary dependencies during next build.
  if (process.env.NEXT_PHASE === 'phase-production-build') {
    dbInstance = {
      all: async () => [],
      get: async () => null,
      run: async () => ({ lastID: 1, changes: 0 }),
      exec: async () => ({ lastID: 1, changes: 0 })
    };
    return dbInstance;
  }
  ...
  if (isPg) {
    try {
      ...
      pool.on('error', (err) => {
        console.warn('PostgreSQL pool idle connection reset (auto-reconnecting):', err.message);
      });
      ...
      const safeQuery = async (queryText, params = []) => {
        try {
          ...
          if (err.message && (err.message.includes('terminated') || err.message.includes('closed') || err.message.includes('ECONNRESET') || err.code === '57P01')) {
            console.warn('PostgreSQL connection lost after server sleep, retrying query...');
      ...
      const tempInstance = {
        all: async (query, params = []) => {
          ...
          const res = await safeQuery(pgQuery, params);
          return res.rows;
        },
        ...
        run: async (query, params = []) => {
          ...
          if (upperQuery.startsWith('BEGIN')) {
            try { await safeQuery('BEGIN'); } catch (e) {}
            return { lastID: null, changes: 0 };
          }
          if (upperQuery.startsWith('COMMIT')) {
            try { await safeQuery('COMMIT'); } catch (e) {}
            return { lastID: null, changes: 0 };
          }
          if (upperQuery.startsWith('ROLLBACK')) {
            try { await safeQuery('ROLLBACK'); } catch (e) {}
            return { lastID: null, changes: 0 };
          }
          ...
          try {
            ...
            const res = await safeQuery(fallbackQuery, params);
            return {
              lastID: null,
              changes: res.rowCount || 0
            };
      ...
      const instructorsCount = await tempInstance.get('SELECT COUNT(*) as count FROM instructors');
  ...
  // --- SQLite Fallback ---
  const dbPath = (process.env.VERCEL || process.env.RENDER || process.env.NEXT_PHASE === 'phase-production-build' || process.env.NODE_ENV === 'production')
  ...
  try {
    ...
    console.warn('SQLite file open failed, using in-memory database:', sqliteErr);
    const { open } = await import('sqlite');
    const sqlite3Module = await import('sqlite3');
    const sqlite3 = sqlite3Module.default || sqlite3Module;
